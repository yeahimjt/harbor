...
export async function userWantsSync(user_id: string) {
  if (!user_id) {
    console.error('No user id provided');
    return;
  }
  try {
    ...
    const userDocSnap = await getDoc(userDoc);
    if (userDocSnap.exists()) {
      const sync = userDocSnap.data().sync;
      return sync;
    }
  } catch (error: any) {
    console.error(
...
export async function handleUserUploadPlaylist(
  user_uid: string,
  ...
  try {
    const usersCollection = collection(firestore, 'users');
    const q = query(usersCollection, where(documentId(), '==', user_uid));
    const usersData = await getDocs(q);
    ...
    if (playlist) {
      const playlistUpload = await fetch(
        `${spotifyBaseUrl}users/${user_id}/playlists`,
        {
          ...
          body: JSON.stringify({
            name: playlist_name,
            description: 'Generated by Harbor',
            public: true,
          }),
        }
      );
      const playlistUploadData = await playlistUpload.json();
      if (playlistUploadData) {
        const track = await handleUserUploadTrack(
          playlistUploadData.id,
          access_token,
          user_id,
          track_uris
        );
    ...
    console.error(`Error handling user upload playlist: `, error);
    return;
  }
}

export async function handleUserUploadTrack(
  playlist_id: string,
  access_token: string,
  user_id: string,
  track_uris: string[]
) {
  return await fetch(`${spotifyBaseUrl}playlists/${playlist_id}/tracks`, {
    method: 'POST',
    headers: {
      Authorization: `Bearer ${access_token}`,
    },

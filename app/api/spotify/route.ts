...
export async function POST(req: Request) {
  ...
  try {
    if (songs) {
      ...
      const playlistSongDataArray = await Promise.all(
        playlistSongFetchPromises
      );
      ...
      if (b64) {
        ...
        const storageRef = ref(
          storage,
          `playlist-covers/${user_id}/${playlists[0].playlistName}`
        );

        const downloadURL = await uploadString(storageRef, b64, 'base64').then(
          () => {
